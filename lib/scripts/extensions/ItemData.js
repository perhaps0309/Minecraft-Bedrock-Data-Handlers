import { EntityComponentTypes, EquipmentSlot, ItemComponentTypes, ItemStack, world } from "@minecraft/server";
...
export class ItemData {
    ...
    constructor(item, player, slot) {
        this.item = item;
        this.player = player;
        this.slot = slot; // You need to set the slot for anything besides lore adjustments.
        const itemEnchantments = this.item.getComponent(ItemComponentTypes.Enchantable);
        const durabilityComponent = this.item.getComponent("durability");
     ...
     * itemData.updateLore();
     */
    removeCustomLore(lore, category) {
        let customLore = this.getDynamicProperty("lore") || {};
        const index = typeof lore === "string" ? lore : lore[0];
    ...
    */
     ...
     * @returns {void}
     */
    setCustomEnchantments(enchantments) {
     ...
     * @returns {void}
     */
    repairItem() { this.setDurability(0); }
    ...
    updateItem() {
        ...
        if (typeof this.slot === "number") {
            ...
            const playerEquipment = this.player.getComponent(EntityComponentTypes.Equippable);
